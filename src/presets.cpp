...
// Menu-item callback. It fires on *every* pick, whereas Fl_Choice's own
// callback only fires when the value actually changes. Re-picking the entry you
// are already on is a real use: it is how you discard hand edits and get the
// preset's values back, and with the widget callback alone that did nothing.
