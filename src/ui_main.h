...
private:
    ...
    // Path the last run actually wrote to. It differs from the fields when the
    // chosen name was taken and got a _1, _2, ... suffix, so the preview after
    // a run has to follow this instead of re-deriving the name.
    std::string m_last_output_path;
