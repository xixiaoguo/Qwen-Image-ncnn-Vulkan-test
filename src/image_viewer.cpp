...
ImageViewer::ImageViewer(int X, int Y, int W, int H, const char *L)
    ...
    m_scroll->scrollbar_size(24); // chunkier scrollbars to match the larger UI
...
void ImageViewer::layout_children() {
    int vw = m_scroll->w() - m_scroll->scrollbar_size();
    int vh = m_scroll->h() - m_scroll->scrollbar_size();

    int iw = m_scaled ? m_scaled->w() : 0;
    int ih = m_scaled ? m_scaled->h() : 0;

    int cw = std::max(iw, vw);
    int ch = std::max(ih, vh);
...
void ImageViewer::zoom_fit() {
    if (!m_source) return;
    int avail_w = std::max(1, m_scroll->w() - m_scroll->scrollbar_size());
    int avail_h = std::max(1, m_scroll->h() - m_scroll->scrollbar_size());
