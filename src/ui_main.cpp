...
void MainWindow::preview_latest_output() {
    // Follow the path the last run actually wrote: when the chosen name was
    // taken it carries a _1, _2, ... suffix the fields do not know about.
    const std::string out = m_last_output_path.empty() ? output_path() : m_last_output_path;
    if (out.empty()) return;
...
void MainWindow::start_generation() {
    ...
    m_last_output_path = opt.output_path;
